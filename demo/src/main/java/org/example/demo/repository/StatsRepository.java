...
public interface StatsRepository extends Repository<Measurement, MeasurementId> {
    ...
    @Query("""
            ...
            FROM Measurement m
            WHERE m.timestamp >= date_trunc('month', :firstDayOfLastMonth)
            AND m.timestamp < date_trunc('month', :firstDayOfLastMonth + 1 month)
            AND exists(select 1 from Measurement inm
                        WHERE inm.cityId = m.cityId
                                    AND inm.timestamp >= date_trunc('day', :firstDayOfLastMonth)
                                    AND inm.timestamp < date_trunc('day', :firstDayOfLastMonth + 1 day))
            AND exists(select 1 from Measurement inm
                        WHERE inm.cityId = m.cityId
                                    AND inm.timestamp >= date_trunc('month', (:firstDayOfLastMonth + 1 month - 1 day))
                                    AND inm.timestamp < date_trunc('month', :firstDayOfLastMonth + 1 month))
            GROUP BY m.cityId
