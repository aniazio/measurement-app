...
public interface StatsRepository extends Repository<Measurement, MeasurementId> {
     ...
     */
    @Query("""
            SELECT new org.example.measurementapp.model.FullStats3h(avg(m.no2), max(m.no2), min(m.no2), avg(m.co), max(m.co), min(m.co), avg(m.pm10), max(m.pm10), min(m.pm10))
                        FROM Measurement m
     ...
     */
    @Query("""
            SELECT new org.example.measurementapp.model.Stats(m.cityId, avg(m.no2))
            FROM Measurement m
